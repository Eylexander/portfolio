...
export default {
    setup() {
        ...
        const items = [
            { 
                name: t('header_bar.home_button'),
                path: "/",
                icon: "solar:home-smile-outline"
            },
            {
                name: t('header_bar.projects_button'),
                path: "/projects",
                icon: "solar:folder-with-files-outline",
            },
            {
                name: t('header_bar.blog_button'),
                path: "/blog",
                icon: "solar:document-add-outline",
            },
            {
                name: t('header_bar.bookmarks_button'),
                path: "/bookmarks",
                icon: "solar:bookmark-linear",
            },
            {
                name: t('header_bar.homelab_button'),
                path: "/homelab",
                icon: "solar:server-square-linear",
            },
            {
                name: t('header_bar.about_button'),
                path: "/about",
                icon: "solar:user-linear",
            },
            {
                name: t('header_bar.contact_button'),
                path: "/contact",
        ...
        watch(locale, () => {
            navItems.value = items.map((item) => {
                return {
                    ...item,
                    name: t(`header_bar.${item.path.split("/")[1] === "" ? "home" : item.path.split("/")[1]}_button`),
                };
            });
        })
